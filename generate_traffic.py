...
class MyTopo(Topo):
    ...
    def build(self):
        ...
        # link s1 with h1, h2, h3
        self.addLink( h1, s1 )
        self.addLink( h2, s1 )
        self.addLink( h3, s1 )

        # link s2 with h4, h5, h6
        self.addLink( h4, s2 )
        self.addLink( h5, s2 )
        self.addLink( h6, s2 )

        # link s3 with h7, h8, h9
        self.addLink( h7, s3 )
        self.addLink( h8, s3 )
        self.addLink( h9, s3 )

        # link s4 with h10, h11, h12
        self.addLink( h10, s4 )
        self.addLink( h11, s4 )
        self.addLink( h12, s4 )

        # link s5 with h13, h14, h15
        self.addLink( h13, s5 )
        self.addLink( h14, s5 )
        self.addLink( h15, s5 )

        # link s6 with h16, h17, h18
        self.addLink( h16, s6 )
        self.addLink( h17, s6 )
        self.addLink( h18, s6 )

        # # link s1 and s2 with s7
        # self.addLink( s1, s7 )
        # self.addLink( s2, s7 )

        # # link s3 and s4 with s8
        # self.addLink( s3, s8 )
        # self.addLink( s4, s8 )

        # # link s5 and s6 with s9
        # self.addLink( s5, s9 )
        # self.addLink( s6, s9 )
...
def startNetwork():
    ...
    topo = MyTopo()
    c0 = RemoteController('c0', ip='13.232.203.92', port=6633)
    ...
    # print("--------------------------------------------------------------------------------")   
    # src.cmd("timeout 20s hping3 -1 -V -d 120 -w 64 -p 80 --fast {}".format(dst))  
    # sleep(100)
